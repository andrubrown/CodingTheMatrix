...
def makeInverseIndex(strlist):
    ...
    """
    dict = {}
    for tuple in enumerate(strlist):
        for word in tuple[1].split():
            if word in dict:
                dict[word] = dict[word] | {tuple[0]}
            else:
                dict[word] = {tuple[0]}
    return dict
...
def orSearch(inverseIndex, query):
    ...
    {0, 2, 3, 4, 5}
    """
    return set(sum([list(inverseIndex[match]) for match in query if match in inverseIndex], []))
...
def andSearch(inverseIndex, query):
    ...
    {0, 4}
    """
    return set.intersection(*[inverseIndex[match] for match in query if match in inverseIndex])
