...
linear_dep_R_2 = [2*math.pi*math.sqrt(2),math.sqrt(2),math.pi]
...
def is_independent(S):
    ...
    True
    '''
    return sum([1 for v in S if is_superfluous(S, v)]) == 0
...
def exchange(S, A, z):
    ...
    '''
    vectors = list(S)
    coefficients = vec2rep(vectors, z)
    for index in range(len(vectors)):
        if coefficients[index] != 0 and not vectors[index] in A:
