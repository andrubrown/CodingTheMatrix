...
def scale_vecs(vecdict):
    ...
    [True, True]
    '''
    return [v/k for k,v in vecdict.items() if k != 0]



## 3: (Problem 3) Constructing span of given vectors over GF(2)
def GF2_span_recurse(D, L):
    if len(L) == 0:
        return {Vec(D, {})}

    sub_span = GF2_span_recurse(D, L[1:])
    return sub_span | {v+L[0] for v in sub_span}
...
def GF2_span(D, S):
    ...
    True
    '''
    return GF2_span_recurse(D, list(S))
