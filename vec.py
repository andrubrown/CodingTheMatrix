...
def getitem(v,k):
    ...
    Be sure getitem(v,k) returns 0 if d is not represented in v.f.
...
def equal(u,v):
    ...
    """
    assert u.D == v.D
    return len({k for k in u.f.keys()|v.f.keys() if getitem(u,k) != getitem(v,k)}) == 0
...
def add(u,v):
    ...
    """
    assert u.D == v.D
    return Vec(u.D, {k:getitem(u,k)+getitem(v,k) for k in u.f.keys()|v.f.keys()})
...
def dot(u,v):
    ...
    """
    assert u.D == v.D
    return sum([getitem(u,k)*getitem(v,k) for k in u.f.keys()&v.f.keys()])
